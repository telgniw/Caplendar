...
class EventHandler(BaseHandler):
    ...
    def _post_new_or_edit_(self):
        ...
        try:
            ...
            event.time = datetime.strptime(time, '%Y-%m-%d %H:%M') if time else event.time
            place_name = self.request.get('event-place-name')
            event.place_name = place_name if place_name else event.place_name
            visibility = self.request.get('event-visibility')
            event.visibility = visibility if visibility else event.visibility
            place = self.request.get('event-place')
            event.place = GeoPt(place) if place else event.place
